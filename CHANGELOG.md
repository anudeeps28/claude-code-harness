...
- **`/to-issues` absorbs `/to-todoist` — one decomposition skill for every backend.** The two skills did the same job and diverged: `/to-todoist` had Todoist's native hierarchy but recorded dependencies as prose in a description, where nothing could read them back, while `/to-issues` had the real blocked-by graph but flattened Todoist to text labels. Merging them upward means a Todoist board now gets **both** — the dependency graph *and* the native features. The two Todoist abilities that justified a separate skill are now expressed through the adapter layer as create-time env overrides (below), so the skill stays backend-agnostic and Todoist stops being a special case. **One deliberate narrowing:** `/to-todoist` grouped work under 1–4 uncompletable milestone parents per run; `/to-issues` creates exactly one parent per run, so several groups means several runs. The dependency graph is the replacement and a better one — ordering is now carried by blocked-by edges a scheduler can read, rather than implied by which milestone bucket a task sat in, which nothing could read. New `--section` (Todoist destination) and `--dry-run` flags carry over from `/to-todoist`; note that `--project` now means different things per backend (a GitHub Projects v2 board, or the destination Todoist project); priority is now a single `p1`–`p4` scale mapped to both a text label and, where the backend has one, a native priority. The hard input gate also now checks the active backend's CLI is actually on PATH before doing any work.
- **Todoist create scripts take `TRACKER_PRIORITY` and `TRACKER_UNCOMPLETABLE`.** `create-issue.sh` and `create-sub-issue.sh` read both from the environment, the same passthrough shape the `ADO_*` overrides already use, because the positional slots are spoken for (arg4 = section, arg5 = project). `TRACKER_PRIORITY` sets Todoist's **native** p1–p4 priority — which sorts and colours the board, unlike a `priority:high` text label — and **fails the create loudly** on an out-of-range value rather than dropping it silently; `TRACKER_UNCOMPLETABLE` creates a task with no checkbox, used for the parent feature so a whole feature can't be ticked off by accident. They are named `TRACKER_*`, not `TODOIST_*`, deliberately: a backend that has no such concept never reads them, so callers set them unconditionally instead of branching on the backend name — branching is how a backend added later silently misses a capability it does have. Defaults are unchanged (both unset → omitted), so existing callers behave exactly as before.

### Removed

- **`/to-todoist` is removed**, folded into `/to-issues` (above). Existing installs drop it on `/update-harness`. Anything that invoked `/to-todoist --project "X" --section "Y"` becomes `/to-issues --project "X" --section "Y"`, and gains the dependency graph it never had. Its `tasks/notes.md` `## Todoist` project/section defaults are read unchanged.
