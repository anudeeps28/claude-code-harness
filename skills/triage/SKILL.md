...
If the user approves, post the triage note in a mode-aware way. **Never use `create-issue.sh` here** — it CREATES A NEW ISSUE and cannot comment on an existing one.

- **Local mode** (`tracker: local`): append the triage note to the task file body `tasks/issues/<ID>.md` (task-specific notes live in the task body, D12).
- **Tracker / Both mode** (external tracker): post as a native comment on the existing issue using the backend's own command (e.g. `gh issue comment <ID> --body ...`, `az boards work-item ... --discussion ...`, `td comment ...`). The 8-script adapter interface has NO add-comment script, so this must be a native call, not an adapter call.
...
- **`todo.md`** — never hand-edited. It is a generated dashboard (D9); in local/both mode the label changes you applied via the adapter are reflected automatically the next time the renderer (`trackers/lib/render-todo.sh`) runs — in local mode `add-label.sh`/`remove-label.sh` already trigger regeneration. In tracker mode `todo.md` does not exist. No manual `todo.md` action in any mode.
- **`flags-and-notes.md`** — if the issue is a high-priority bug or blocker, add to "Active Blockers":
  ...
  ```
  `flags-and-notes.md` ships only in the team pack. If it is absent (the solo pack ships only `notes.md` + `tracker-config.md`), route the same `- [TRIAGE] ...` line to `tasks/notes.md` instead (D12: ad-hoc scratch → notes.md). Keep the same line format.
